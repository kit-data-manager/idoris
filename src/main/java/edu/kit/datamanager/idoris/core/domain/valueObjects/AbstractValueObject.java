...
public abstract class AbstractValueObject<T> {
    private final T unvalidated;

    @JsonCreator
    public AbstractValueObject(T unvalidated) {
        if (unvalidated == null) {
            throw new NullPointerException("Value object cannot be null");
        }
        this.unvalidated = unvalidated;
    }

    protected T getUnvalidated() {
