...
package edu.kit.datamanager.idoris.metadata.pids.services;
...
import edu.kit.datamanager.idoris.metadata.pids.client.model.PIDRecordEntry;
import edu.kit.datamanager.idoris.metadata.pids.dao.IPersistentIdentifierDao;
import edu.kit.datamanager.idoris.metadata.pids.domain.PIDNode;
import edu.kit.datamanager.idoris.metadata.pids.domain.TypedPIDMakerException;
import edu.kit.datamanager.idoris.metadata.pids.utils.PIDRecordMapper;
...
public class PersistentIdentifierService implements IInternalPIDService {
    ...
    public PIDNode createPersistentIdentifier(@SpanAttribute AdministrativeMetadata entity) {
        ...
        List<PIDRecordEntry> entries = createdRecord.record().stream()
                .map(entry -> {
                    if (Objects.equals(entry.key().toString(), "21.T11148/b8457812905b83046284")) {
